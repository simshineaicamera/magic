...
def detection(iou_threshold, confidence_threshold, input_names):
    ...
    batch_size = 4 #len(input_names)
    
    total = int(len(input_names)/batch_size)
    print("total: ",total)
    ...
    with tf.Session() as sess:
        ...
        for kk in range(total):
            images = input_names[4*kk:4*kk+4]
            batch = load_images(images, model_size=_MODEL_SIZE)
            detection_result.append(sess.run(detections, feed_dict={inputs: batch}))
            image_list.append(images)
           # all_detection.append(detection_result)
    #print("result is: ",detection_result)
    return image_list, detection_result, _MODEL_SIZE
